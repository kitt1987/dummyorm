...
class DDLEngine extends PrivateFuncHelper {
  ...
  syncSchema(step, schema) {
    ...
    if (create) seq.push(this._bind(createTable, create, schema));
    if (add) seq.push(this._bind(addColumns, add, schema));
    if (modify) seq.push(this._bind(modifyColumns, modify, schema));
    if (drop) seq.push(this._bind(dropColumns, drop, schema));
    if (addIndex) seq.push(this._bind(addIndicies, addIndex, schema));
    if (dropIndex) seq.push(this._bind(dropIndices, dropIndex, schema));
    if (addFK) seq.push(this._bind(addForeignKey, addFK, schema));
    if (dropFK) seq.push(this._bind(dropForeignKey, dropFK, schema));
    if (addFTcolumn)
      seq.push(this._bind(addFullTextColumns, addFTcolumn, schema));
