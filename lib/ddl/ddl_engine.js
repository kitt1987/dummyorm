...
function _createTable(createObj, schema, acb) {
  ...
  this.engine.performSQL(sql, acb);
}
...
function _describeIndex(indexObj) {
  ...
  if (indexObj.type) {
    columnDef = util.format('ADD INDEX %s %s (%s)', indexObj.name,
      indexObj.type.sql, indexObj.column.map('sqlName').join(','));
  } else {
    columnDef = util.format('ADD INDEX %s (%s)', indexObj.name,
      indexObj.column.map('sqlName').join(','));
  }
...
exports.prototype.syncSchema = function(step, schema, cb) {
  ...
  if (create) seq.push(_createTable.bind(this, create, schema));
  if (add) seq.push(_addColumns.bind(this, add, schema));
  if (modify) seq.push(_modifyColumns.bind(this, modify, schema));
  if (drop) seq.push(_dropColumns.bind(this, drop, schema));
  if (addIndex) seq.push(_addIndicies.bind(this, addIndex, schema));
  if (dropIndex) seq.push(_dropIndices.bind(this, dropIndex, schema));
  if (addFK) seq.push(_addForeignKey.bind(this, addFK, schema));
  if (dropFK) seq.push(_dropForeignKey.bind(this, dropFK, schema));
  if (addFTcolumn)
...
function _checkDDLTable() {
  var ddlCreation = 'CREATE TABLE IF NOT EXISTS ' +
    ...
    exports.stepColumn + ' BIGINT NOT NULL) ENGINE=InnoDB;';
  return this.engine.performSQL(ddlCreation);
}

function _findSyncStep(cb) {
  var queryStep = 'SELECT ' + exports.stepColumn + ' FROM ' +
    this.ddlTableName() + ';';
  return this.engine.performSQL(queryStep)
    .then((result) => {
      ...
      if (result.length == 1) {
        logger.info(colors.green('The last step is ' +
          result[0][exports.stepColumn]));
        return result[0][exports.stepColumn];
      }
...
exports.prototype.switchDB = function(db) {
  return _checkDB(db)
    .then(() => _switchDB(db))
    .then(() => _checkDDLTable())
    .then(() => _findSyncStep());
