...
exports.stepColumn = 'step_ts';

exports.prototype.ddlTableName = function() {
...
function _describeColumn(columnObj) {
  if (!columnObj.sqlName)
    throw new Error('The column sqlName is requisite ' + util.inspect(columnObj));
  ...
  var columnDef = '';
  if (columnObj.len) {
    columnDef = util.format('%s %s(%d)', columnObj.sqlName, columnObj.type.sql, columnObj.len);
  } else {
...
function _createTable(createObj, schema, acb) {
  ...
  if (createObj.options) {
    if (createObj.options.engine)
      engine = createObj.options.engine;
...
function _addFullTextColumns(fullTextColumns, schema, acb) {
  if (fullTextColumns.length === 0)
    throw new Error('You should define 1 column at least');

  var columns = fullTextColumns.join(',');

  var sql = util.format('ALTER TABLE %s ADD FULLTEXT (%s);', schema.tableName, columns);
  this.engine.performSQL(sql, function(err) {
    acb(err);
  });
}
...
function _dropIndices(dropIndexObj, schema, acb) {
  ...
  var indecies = _.map(dropIndexObj, function(index) {
...
function _addForeignKey(fkObjs, schema, acb) {
  ...
  var fks = _.map(fkObjs, _describeAddForeignKey.bind(null, schema.tableName)).join(',');
...
function _dropForeignKey(fkObjs, schema, acb) {
  ...
  var fks = _.map(fkObjs, _describeDropForeignKey.bind(null, schema.tableName)).join(',');
  var sql = util.format('ALTER TABLE %s %s', schema.tableName, fks);
  this.engine.performSQL(sql, function(err) {
    acb(err);
  });
...
function _updateSyncStep(step, cb) {
  var up = 'UPDATE ' + this.ddlTableName() + ' SET ' + exports.stepColumn +
    '=' + step + ';';
...
exports.prototype.syncSchema = function(step, schema, cb) {
  ...
  if (addFTcolumn)
    seq.push(_addFullTextColumns.bind(this, addFTcolumn, schema));
...
function _checkDB(db, cb) {
  var dbCreation = 'CREATE DATABASE IF NOT EXISTS ' + db + ' CHARACTER SET utf8;';
...
function _switchDB(db, cb) {
  ...
  this.engine.useDB(db, cb);
}

function _checkDDLTable(cb) {
  var ddlCreation = 'CREATE TABLE IF NOT EXISTS ' +
    this.ddlTableName() + '(id INT PRIMARY KEY AUTO_INCREMENT, ' + exports.stepColumn +
...
function _findSyncStep(cb) {
  var queryStep = 'SELECT ' + exports.stepColumn + ' FROM ' + this.ddlTableName() + ';';
  this.engine.performSQL(queryStep, function(err, result) {
    ...
    if (result.length == 1) {
      logger.info(colors.green('The last step is ' + result[0][exports.stepColumn]));
      cb(err, result[0][exports.stepColumn]);
      return;
    }

    this.engine.performSQL('INSERT INTO ' + this.ddlTableName() +
      '(' + exports.stepColumn + ') VALUES(0);',
      function(err, result) {
        cb(err, 0);
      });
  }.bind(this));
