...
function _describeForeignKey(fkObj) {
	var column = {
	...
	var fkDef = util.format('FOREIGN KEY (%s) REFERENCES %s(%s)', column.name,
			fkObj.tableName, fkObj.id.name);
...
function _addForeignKey(fkObjs, schema, acb) {
	...
	var fks = _.map(fkObjs, _describeForeignKey).join(',');
