...
function _createTable(createObj, schema, acb) {
	...
	var engine = createObj.engine || 'InnoDB';
	var sql = util.format('CREATE TABLE IF NOT EXISTS %s(%s) ENGINE=%s;',
		createObj.table, columns, engine);
...
function _describeAddForeignKey(fkObj) {
	var columnName = fkObj.tableName + '_id';
	var fkDef = util.format('CONSTRAINT %s FOREIGN KEY (%s) REFERENCES %s(%s)',
		'fk_' + columnName, columnName, fkObj.tableName, fkObj.id.name);
	return 'ADD ' + fkDef;
}
