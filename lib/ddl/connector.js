...
class Connector {
    ...
    return this.storage.connect()
      .then(() => {
        ...
        return ddlEngine.switchDB(options.db)
          .then(
            (syncTs) => this.loadSchema(options.schemaPath, options.db, ddl,
              ddlEngine, syncTs)
          ).then(() => ddl.schema);
      });
  }
  ...
  loadSchema(stepPath, database, ddl, ddlEngine, syncTs) {
    ...
    var stepLoader = stepModules.map((step) => {
      return () => {
        ...
        return new Promise((resolve, reject) => {
          ...
          if (schemas.length === 0) {
            this.logger.warn('No schema defined in ' + step.name);
            resolve();
            return;
          }
          ...
          if (step.stepTs <= syncTs) {
            this.logger.warn(util.format(
              'Skip sync ' + step.name + ' cuz its stepTs(%d) <= syncTs(%d)',
              step.stepTs, syncTs));
          ...
          resolve(utilLocal.chainPromise(syncSchema));
        });

      }
    });

    return utilLocal.chainPromise(stepLoader);
  }
}
