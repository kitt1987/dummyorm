	...
	var stepChain = _.map(steps, function(s) {
		return function(syncTs, lastStep, acb) {
			...
			step.run(this, function(schemas) {
				...
				if (stepTs <= syncTs) {
					logger.warn(colors.yellow(util.format('The schemas will not sync \
				...
				async.series(_.map(schemas, function(schema) {
					return function(cb) {
						this.ddlEngine.syncSchema(schema, function(err) {
							if (err) {
								logger.error(colors.red('Fail to sync schemas due to ' + err));
								cb(err);
								return;
							}
							...
							_updateSyncStep.bind(this, stepTs)(cb);
						}.bind(this));
					}.bind(this);
