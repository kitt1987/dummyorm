...
class Schema extends DML {
  ...
  create() { /* properties */
    if (!arguments)
      throw new Error(
        'ORM try to create an empty record of ' + this.schema.tableName
      );

    var props = {}, columns = {};
    arguments.forEach((prop) => Object.assign(props, prop));
  ...
  outline(record, schemaFinder) {
    ...
    Object.keys(record).forIn((v, k) => {
      ...
      if (column && column.type === datatype.ForeignKey) {
        validateForeignKey(v);
        const fkSchema = schemaFinder(v.schemaName);
        dump[k] = fkSchema.outline(v, schemaFinder);
...
function editColumn(column) {
  if (!this.columns[column.name]) throw new Error('No column:' + column.name);
  return newColumn(this, column);
}
