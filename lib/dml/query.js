...
exports.prototype.function = function(func) {
	this._select = func;
}

exports.prototype.join = function(schema, condition) {
	if (this._join) {
		this._join += ' INNER JOIN ' + schema.tableName + ' on ' + condition;
	} else {
		this._join = 'INNER JOIN ' + schema.tableName + ' on ' + condition;
	}
...
exports.prototype.leftJoin = function(schema, condition) {
	if (this._join) {
		this._join += ' LEFT JOIN ' + schema.tableName + ' on ' + condition;
	} else {
		this._join = 'LEFT JOIN ' + schema.tableName + ' on ' + condition;
	}
...
exports.prototype.rightJoin = function(schema, condition) {
	if (this._join) {
		this._join += ' RIGHT JOIN ' + schema.tableName + ' on ' + condition;
	} else {
		this._join = 'RIGHT JOIN ' + schema.tableName + ' on ' + condition;
	}
