'use strict';

var Query = require('./query');
var Condition = require('./condition');
var Transaction = require('./transaction');
var ddl = require('../ddl/datatype');
var _ = require('lodash');
var u = require('../utility');

exports = module.exports = {};

function calcCacheKey(key, record) {
	if (typeof key === 'function') {
		return key(record);
	}

	return key;
}

function _updateCache(key, record, cb) {
	var cacheKey = calcCacheKey(key, record);
	this.cache.get(cacheKey, function(err, result) {
		if (err) {
			cb(err);
			return;
		}

		if (result && result[ddl.UpdateTsField] >= record[ddl.UpdateTsField]) {
			cb();
			return;
		}

		this.cache.keep(cacheKey, record.dump(), cb);
	}.bind(this));
}

exports.save = function(key, record, cb) {
	record[ddl.UpdateTsField] = _.now();
	var self = this;
	this.storage.insert(record, function(err) {
		if (err) {
			cb(err);
			return;
		}

		if (self.cache && key) {
			_updateCache.bind(self)(key, record, cb);
		} else {
			cb();
		}
	});
};

exports.update = function(key, record, cb) {
	record[ddl.UpdateTsField] = _.now();
	var self = this;
	this.storage.update(record, function(err) {
		if (err) {
			cb(err);
			return;
		}

		if (self.cache && key) {
			_updateCache.bind(self)(key, record, cb);
		} else {
			cb();
		}
	});
};

exports.del = function(key, record, cb) {
	var self = this;
	this.storage.del(record, function(err) {
		if (self.cache && key) {
			self.cache.del(key, cb);
		} else {
			cb();
		}
	});
};

// Cache accessor
exports.keep = function(key, record, cb) {
	if (!this.cache)
		throw new Error('Cache is inactive');
	this.cache.keep(key, record.dump(), cb);
};

exports.keepSimple = function(key, obj, cb) {
	if (!this.cache)
		throw new Error('Cache is inactive');
	this.cache.keep(key, JSON.stringify(obj), cb);
};

exports.get = function(key, cb) {
	if (!this.cache) {
		cb(new Error('Cache is inactive'));
		return;
	}
	this.cache.get(key, cb);
};

exports.remove = function(key, cb) {
	if (!this.cache) {
		cb(new Error('Cache is inactive'));
		return;
	}

	this.cache.del(key, cb);
};

exports.query = function(schemas) {
	if (schemas) {
		schemas = u.toArray(schemas);
		return new Query(this.storage, _.map(schemas, 'tableName'));
	} else {
		return new Query(this.storage);
	}
};

exports.transaction = function() {
	return new Transaction(this, exports);
}

exports.sql = function(sql, cb) {
	this.storage.performSQL(sql, cb);
}