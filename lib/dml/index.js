...
function __updateCache(record, key, variation, cb) {
	this.cache.gets(key, (err, data) => {
		...
		this.cache.cas(key, record, data.cas, err => {
			if (!err) {
...
exports.update = function(key, record, variation, cb) {
  var theKey = typeof key === 'function' ? key(record.id) : key;
  if (this.cache && theKey) {
		__updateCache.call(this, record, theKey, variation, cb);
    return;
  }
